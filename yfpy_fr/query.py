...
from yfpy_fr.exceptions import YahooFantasySportsDataNotFound
from yfpy_fr.logger import get_logger
# MADE CHANGES
# added 'Stat' to import
from yfpy_fr.models import YahooFantasyObject, DraftResult, Game, GameWeek, User, League, Standings, Settings, Player, \
    PositionType, StatCategories, Transaction, Scoreboard, Team, TeamPoints, TeamProjectedPoints, TeamStandings, \
    Roster, RosterPosition, Matchup, Stat
from yfpy_fr.utils import jsonify_data, prettify_data, reformat_json_list, unpack_data
...
class YahooFantasySportsQuery(object):
    ...
    def __init__(self, auth_dir: Union[Path, str], league_id: str, game_code: str, game_id: int = None,
                 offline: bool = False, all_output_as_json_str: bool = False, consumer_key: str = None,
                 consumer_secret: str = None, browser_callback: bool = not runtime_environment_is_docker,
    ...
    def query(self, url: str, data_key_list: Union[List[str], List[List[str]]], data_type_class: Type = None,
        ...
        if not self.offline:
            ...
            self.json = response.json().get(self.fantasy_content_data_field)
    ...
    def get_league_key(self, season: int = None) -> str:
        ...
        if not self.league_key:
            # print("wasn't found")
            if season:
                self.league_key = f"{self.get_game_key_by_season(season)}.l.{self.league_id}" ## added this line
                # return f"{self.get_game_key_by_season(season)}.l.{self.league_id}" ## original line
                return self.league_key
            elif self.game_id:
                self.league_key = f"{self.get_game_metadata_by_game_id(self.game_id).game_key}.l.{self.league_id}" ## added this line
                # return f"{self.get_game_metadata_by_game_id(self.game_id).game_key}.l.{self.league_id}" ## original line
                return self.league_key
            else:
                # print("else")
                logger.warning(
                    "No game id or season/year provided, defaulting to current fantasy season.")
                self.league_key = f"{self.get_current_game_metadata().game_key}.l.{self.league_id}"
                # return f"{self.get_current_game_metadata().game_key}.l.{self.league_id}" ## original line
                return self.league_key
        else:
            return self.league_key
    ...
    def get_team_stats_by_week(
        ...
        Returns:
            list[Stat]: List containing YFPY Stat objects
        ...
        # MADE CHANGES
        # changed data_key_list to ["team","team_stats","stats"]
        return self.query(
            f"https://fantasysports.yahooapis.com/fantasy/v2/team/{team_key}/stats;type=week;week={chosen_week}",
    ...
    # MADE CHANGES
    # new function to batch get stats in a given week for ALL teams in league
    def get_all_team_stats_by_week(
            self, chosen_week: Union[int, str] = "current"
    ) -> Dict[int, Dict[int, int]]:
        """Retrieve stats of ALL teams by team_id and by week for chosen league.
        ...
        Returns:
            dict[team_id, dict[stat_id, stat_value]]: Dictionary containing team_id keys and dictionary values
            containing a stat_id key and a stat_value value.

        """
        league_key = self.get_league_key()
        team_keys = [f"{league_key}.t.{team.team_id}" for team in self.get_league_teams()]
        ...
        team_stats = {team.team_id: {stat['stat'].stat_id:stat['stat'].value for stat in team.team_stats} for team in query}
        return team_stats

    def test_func(self, team_id):
        team_key = f"{self.get_league_key()}.t.{team_id}"
        # return self.query(
        #     f"https://fantasysports.yahooapis.com/fantasy/v2/team/{team_key}/stats",
        #     ["team_points"]
